...
while True:
    ...
    try:
        ...
        #####FIND GRADEPOINT###########
        gradePoint = ulits.getCornerPoints(rectCon[4])
        # print(gradePoint)

        #####FIND NAME BOX###########
        nameBox = ulits.getCornerPoints(rectCon[3])
        # print(nameBox)
        # cv2.rectangle(img, (190,84), (576, 100),(255,0,0), 1 )
        ...
        if leftBigCon.size != 0 and gradePoint.size != 0 and rightBigCon.size !=0:
            ...
            imgResultL = ulits.showAnswers(imgWarpcoloredL, myIndexL, gradingL, ans1, int(questions/2), choices)
            imgRawDrawingL = np.zeros_like(imgWarpcoloredL)
            imgRawDrawingL = ulits.showAnswers(imgRawDrawingL, myIndexL, gradingL, ans1, int(questions/2), choices)
            invMatrixL = cv2.getPerspectiveTransform(pointL2, pointL1)
            imgInvWarpL = cv2.warpPerspective(imgRawDrawingL, invMatrixL, (widthImg, heigthImg))
    ...
    # cv2.imshow("Test", img)
    # cv2.imshow("Result", imgWarpName)
    cv2.imshow("ss", imgFinal)
    if cv2.waitKey(1) & 0xFF == ord('s'):
        cv2.imwrite("bai1.png", imgFinal)
        cv2.waitKey(300)
